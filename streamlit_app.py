    ...
    if st.button("Показать модели"):
        with st.spinner("Обращаемся к Minio..."):
            try:
                ...
                objects = client.list_objects(MINIO_BUCKET_NAME, prefix="models/", recursive=True)
    ...
    if st.button("Удалить модель"):
        with st.spinner("Удаляем модель..."):
            try:
                ...
                client.remove_object(MINIO_BUCKET_NAME, f"models/{model_to_delete}")
    ...
    if st.button("Показать данные", key='find_to_drop_data'):
        with st.spinner("Обращаемся к Minio..."):
            try:
                ...
                objects = client.list_objects(MINIO_BUCKET_NAME, prefix="datasets/", recursive=True)
    ...
    if st.button("Удалить данные"):
        with st.spinner("Удаляем данные..."):
            try:
                ...
                client.remove_object(MINIO_BUCKET_NAME, f"datasets/{data_to_delete}")
